...
- 게임일시정지 기능 추가(단축키:p)
- 적들도 HP를 갖도록 수정
- 적군 2종류로 확장 : 박쥐 => (공격속도가 느리고 생명력이 강함),
                     벌  => (공격속도가 빠르고 생명력이 약함)
- 각종 전역함수들 오브젝트에 종속시킴
- 플레이어가 마우스를 누르고 있으면 총알 자동 발사
- 몬스터 채력바 구현
# 에니메이션 적용
- 액터 이미지들을 스프라이트 이미지로 사용하기
- 액터의 공격방항에 따라서 스프라이트 4방향처리
- 액터 스프라이트 에니메이션 (이동) 구현
- 플레이어는 이동할때만 이동에니메이션 처리
# 맵 오브젝트 충돌 처리
