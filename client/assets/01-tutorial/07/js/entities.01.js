...
Player = function(){
    ...
    self.updatePosition = function() {
        ...
        if( self.x < self.width/2 )
            self.x = self.width/2;
        if( self.x > Map.current.width-self.width/2 )
            self.x = Map.current.width-self.width/2;
        if( self.y < self.height/2 )
            self.y = self.height/2;
        if( self.y > Map.current.height-self.height/2 )
            self.y = Map.current.height-self.height/2;
    }
    // 갱신
    var super_update = self.update;
    self.update = function() {
...
Enemy.randomlyGenerate = function() {
    var id      = Math.random();
    var x       = Math.random() * Map.current.width;
    var y       = Math.random() * Map.current.height;
    var width   = 64;
    var height  = 64;
    if( Math.random() < 0.5 ) {
...
Bullet = function ( id, x, y, speedX, speedY, width, height, combatType ) {
    ...
    self.updatePosition = function() {
        ...
        if( self.x < 0 || self.x > Map.current.width ) {
            self.speedX = -self.speedX;
        }
        if( self.y < 0 || self.y > Map.current.height ) {
